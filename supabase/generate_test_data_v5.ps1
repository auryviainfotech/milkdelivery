...
Write-Host "Creating CORRECTED orders for UUID $uuid..." -ForegroundColor Cyan

# 1. Create Orders (Corrected Schema: user_id, delivery_date, status, created_at)
# Omitted: product_name, quantity, subscription_id
Invoke-SQL "
INSERT INTO public.orders (user_id, delivery_date, status, created_at)
VALUES 
('$uuid', '$today', 'pending', now()),
('$uuid', '$today', 'pending', now())
